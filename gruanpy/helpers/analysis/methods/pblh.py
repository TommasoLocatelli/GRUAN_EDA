...
class PBLHMethods:
    ...
    def parcel_method(self, data):
        ...
        index = data[data['virtual_potential_temp'] < surface_virtual_potential_temperature].index
        if not index.empty:
            pblh_index = index[0]
    ...
    def richardson_number_method(self, data):
        ...
        data['Ri_b'] = FM.bulk_richardson_number(data['avg_Tv'], data['delta_virtual_potential_temp'], data['delta_z'], data['delta_wspeed'])
        # apply criterion
        data['pblh_Ri'] = 0
        index = data[data['Ri_b'] > 0.25].index
        if not index.empty:
            pblh_index = index[0]
